...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope media-cli
...
scope:** the default personal scope is **private** (`--scope media-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `media-cli`). Private records are served only to a recall in the same
