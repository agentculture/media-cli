...
media-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. media-cli
...
#   - consumer-identifying prose: `guildmaster` → `media-cli` (NOT
...
If a re-sync would lose a media-cli adaptation, lift the change
...
#     → `which media-cli vendors from guildmaster`
