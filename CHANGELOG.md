...
- **Independently re-verified host survey (2026-07-24)** recorded in `CLAUDE.md`, extending or correcting the brief on three points. (1) *PipeWire object IDs are unstable too* — the brief recorded the HDMI sink as id `115`; it reads `130` today, within the same day, while the Reachy sink held at `51`. That is a fourth unstable numbering scheme alongside `/dev/videoN`, ALSA card numbers, and PortAudio indices; the `alsa_output.usb-…` node name is the stable handle. (2) *The seat-ACL hazard is asymmetric between video and audio*, which the brief flattens: `/dev/video0` is group `video` and the operator is not a member, so the logind seat ACL is the only grant, whereas `/dev/snd/controlC0` is group `audio` and the operator **is** a member — so a headless agent, container, or systemd unit loses video access but keeps audio access on this host. (3) *`paplay` and `pw-play` are present* alongside `aplay` and `speaker-test`, a small addition to the brief's binary inventory; `ffmpeg`, `sox`, `v4l2-ctl`, and `fswebcam` remain absent as recorded.
- **Open question 2 (the `harmonics-cli` seam) sharpened from hypothetical to evidenced.** `harmonics-cli` 0.8.0 has already shipped live playback with device selection — `--device` (name substring or PortAudio index), `$HARMONICS_AUDIO_DEVICE`, and `sounddevice` enumeration in `harmonics/audio/_playback.py::select_output_device`. Its default is a hardcoded name-substring guess (prefer `pipewire`/`pulse`), and its own docstring gives the reason: a host whose default sink is "a FIXED-rate device (e.g. a 16 kHz USB audio adapter)" rejects the synth's 44.1 kHz. That adapter is this host's Reachy Mini sink, which is also the default sink — so a sibling hit media-cli's format-negotiation problem in production and worked around it with a guess. `CLAUDE.md` records this as the strongest available argument that the lane holds, and names the split to propose (harmonics decides *what* to play; media-cli resolves *which device* and what it can accept).
- **Three scaffold defects and two open genesis items documented** rather than silently carried. Defects: the self-description strings in `learn`, `explain/catalog.py`, `overview`, and the parser description still call this repo "a clonable template for AgentCulture mesh agents"; the console command is `media` while `prog="media-cli"`, so every hint and the whole catalog instruct agents to type a command that is not installed (`harmonics-cli` fixed exactly this in its own issue #2); and the catalog's `("media",)` key is load-bearing for the rubric's `explain_self` check, so a `prog` fix must keep both keys. Open items: the backend is unreconciled (issue #1's table says `claude` *(seed)*, `culture.yaml` says `colleague`, and because both prompt files exist on disk `doctor` stays green either way and will not catch a half-done switch), and `teken` has been renamed to `agentfront` upstream while CI pins the pre-rename `teken 0.8.0` under a `teken>=0.8` specifier, so a `uv lock --upgrade` can break the rubric gate.

### Changed

- **`CLAUDE.md` re-initialized** from the self-init seed into a full runtime prompt, per the build brief ([#1](https://github.com/agentculture/media-cli/issues/1)) and modeled on sibling `webcam-cli`'s structure. Now covers: the device-plane lane and an explicit out-of-lane table (capture → `webcam-cli`, *what* to play → `harmonics-cli`, interpretation → neither); the scaffold-vs-product state; identity and the unreconciled backend; verified build/test/lint/rubric commands; the cited-CLI architecture, the four places a new verb touches, and the two stable contracts; domain constraints re-verified on the operator's host; the six open design questions; conventions and layout.
- **`README.md` rewritten** from `culture-agent-template` scaffold prose into a description of the actual agent. Drops the "Make it your own" template-cloning section, adds a "Why a device plane" section arguing the lane from concrete host evidence, states the scaffold status honestly up front, and **corrects the quickstart to the installed console command `media`** — it previously instructed `uv run media-cli whoami`, which fails with "Failed to spawn" because no such binary is installed.
...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`media-cli vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/media-cli/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `media_cli/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=media_cli` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/media-cli/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/media-cli/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: media-cli`,
